...
- [x] ~~**オフライン中のログアウト**~~ — **仕様変更（2026-09-13、PR 7 の実機テストで決定）**。オフラインモード中の画面は「Back to Online Mode」だけを出し、アカウント操作は表示しない。したがって *オフライン中にログアウトする導線は存在しない*。ユーザーが実機で確認したうえで「今の動作のほうが分かりやすい」と判断したため、この挙動を正とする。
  `useAuth.logout()` 側は引き続き安全側（サーバ呼び出しは best-effort、ダウンロード済みファイルは削除しない）に倒してあるので、将来この導線を足す場合も再生中のファイルを消す事故は起きない。
