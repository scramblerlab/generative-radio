...
# Test 2: JWT "none" algorithm bypass (token built at runtime — not a stored credential)
NONE_HDR=$(printf '{"alg":"none"}' | base64 | tr -d '=\n' | tr '+/' '-_')
NONE_PAY=$(printf '{"sub":"1234","name":"Admin","role":"admin"}' | base64 | tr -d '=\n' | tr '+/' '-_')
NONE_TOKEN="${NONE_HDR}.${NONE_PAY}."
...
for i in $(seq 1 5); do
  curl -so /dev/null -w "%{http_code} " \
    ...
    -H "Content-Type: application/json" \
    -d "{\"email\":\"spam$i@example.com\",\"password\":\"test\"}"
done; echo
...
# Token built at runtime — not a stored credential
TRUNC_HDR=$(printf '{"alg":"HS256","typ":"JWT"}' | base64 | tr -d '=\n' | tr '+/' '-_')
TRUNC_PAY=$(printf '{"sub":"1234567890","role":"admin"}' | base64 | tr -d '=\n' | tr '+/' '-_')
TRUNC_TOKEN="${TRUNC_HDR}.${TRUNC_PAY}."  # no signature — tests fail-open behaviour
...
*Advanced patterns (sections 15–21) are in `patterns-advanced.md`.*
