...
class RadioOrchestrator:
    def __init__(self, llm: OllamaClient, acestep: ACEStepClient):
        ...
        # Prompt prefetch: the next track's LLM prompt is prepared while ACE-Step
        # synthesizes the current one (LLM ~25s overlaps ACE-Step ~73s of I/O wait).
        self._prompt_task: asyncio.Task | None = None
        self._next_prompt: tuple[SongPrompt, str, list[str]] | None = None  # (prompt, genre_label, genre_ids)
    ...
    async def start(
        ...
        self._invalidate_prepared_prompt()
    ...
    def _apply_pending_settings(self) -> None:
        ...
        self.history = []
        # A prompt prefetched under the old settings is now stale — drop it so the
        # next generation re-runs the LLM with the new genres/keywords/language.
        self._invalidate_prepared_prompt()
        logger.info(
    ...
    async def _radio_loop(self) -> None:
        try:
            ...
            # Instant start: replay a library track while the first fresh track
            # is generated by the background chain. Falls back to the original
            # generate-and-wait path when the library is empty/unavailable.
            # Done BEFORE the ACE-Step health check — replays don't need ACE-Step.
            replay = await self._make_replay_track()
            if replay is not None:
                logger.info(f"[radio] Instant start — replaying '{replay.song_title}' from library")
                self.current_track = replay
                self.state = RadioState.PLAYING
                await self._broadcast_track_ready(replay, is_next=False)
                await self._broadcast_status("playing", "Replaying from library — generating a fresh track...")
                self._start_play_now_watchdog(replay)

            # Verify ACE-Step is reachable. Its /health endpoint can go silent
            # for several seconds while the MLX LM is mid-generation (e.g. the
            # startup warmup), so retry before declaring it down.
            healthy = False
            for attempt in range(12):  # ~60s worst case
                if await self.acestep.health_check():
                    healthy = True
                    break
                if self._stop_event.is_set():
                    return
                logger.info(f"[radio] ACE-Step health check attempt {attempt + 1}/12 failed — retrying in 5s")
            ...
            # Start pre-generating upcoming tracks in the background
            self._start_prebuffer()
            ...
            while not self._stop_event.is_set():
                ...
                if self.next_track is not None:
                    ...
                    # Next track still generating. Prefer replaying another library
                    # track over a silent buffering gap.
                    replay = await self._make_replay_track()
                    if replay is not None:
                        old_id = self.current_track.id if self.current_track else None
                        self.current_track = replay
                        if old_id:
                            ...
                            self._purge_evicted_tracks()
                        logger.info(
                            f"[pipeline] Gap-fill: replaying '{replay.song_title}' from library "
                            f"while generation catches up {self._pipeline_str()}"
                        )
                        self.state = RadioState.PLAYING
                        await self._broadcast_track_ready(replay, is_next=False)
                        await self._broadcast_status("playing", "Replaying from library — still generating...")
                        self._start_play_now_watchdog(replay)
                        self._start_prebuffer()
                        continue
    ...
    def _invalidate_prepared_prompt(self) -> None:
        """Drop any prefetched prompt — it was built with now-stale settings."""
        if self._prompt_task and not self._prompt_task.done():
            self._prompt_task.cancel()
        self._prompt_task = None
        if self._next_prompt is not None:
            logger.info(
                f"[pipeline] Discarding prefetched prompt '{self._next_prompt[0].song_title}'"
            )
            self._next_prompt = None

    async def _prefetch_next_prompt(self) -> None:
        """Prepare the next track's prompt while ACE-Step synthesizes the current one."""
        try:
            prepared = await self._prepare_song_prompt(quiet=True)
            if self._stop_event.is_set():
                return
            self._next_prompt = prepared
            logger.info(f"[pipeline] Prompt prefetched: '{prepared[0].song_title}'")
        except asyncio.CancelledError:
            raise
        except Exception as e:
            logger.warning(f"[pipeline] Prompt prefetch failed — will generate inline: {e}")

    async def _prepare_song_prompt(
        self, short_id: str | None = None, quiet: bool = False
    ) -> tuple[SongPrompt, str, list[str]]:
        """Step 1: the LLM writes a structured song prompt.

        Returns (song_prompt, genre_label, genre_ids). The title is appended to
        history here — not after synthesis — so a prompt prefetched during the
        previous track's synthesis still sees that track's title.
        quiet=True suppresses progress broadcasts (used by the prefetch path).
        """
        short_id = short_id or str(uuid.uuid4())[:8]
        track_index = len(self.history)
        target_duration = get_progressive_duration(track_index)
        logger.info(
            f"[radio] [{short_id}] Preparing song prompt "
        ...
        return song_prompt, genre_label, genres_for_llm
    ...
    async def _generate_track(self) -> TrackInfo:
        ...
        # Step 1: structured song prompt — use the one prefetched during the
        # previous track's synthesis when available, else call the LLM now.
        if self._prompt_task and not self._prompt_task.done():
            logger.info(f"[radio] [{short_id}] Waiting for in-flight prompt prefetch...")
            with contextlib.suppress(Exception):
                await self._prompt_task
        self._prompt_task = None

        prepared = self._next_prompt
        self._next_prompt = None
        if prepared is not None:
            song_prompt, genre_label, genres_for_track = prepared
            logger.info(
                f"[radio] [{short_id}] Using prefetched prompt — "
                f"'{song_prompt.song_title}' | {song_prompt.bpm} BPM | {song_prompt.key_scale}"
            )
            await self._broadcast_progress(
                "llm_done",
                f'"{song_prompt.song_title}" · {song_prompt.bpm} BPM · {song_prompt.key_scale}',
                {
                    ...
                    "bpm": song_prompt.bpm,
                    "key": song_prompt.key_scale,
                    "llmSeconds": 0.0,  # prepared concurrently with the previous track
                },
            )
        else:
        ...
        # Persist to the library: adopt (move) ACE-Step's original api_audio file —
        # never a second copy. Enables instant-start replays on future sessions.
        asyncio.create_task(self.library.adopt(
            track_id,
            result_meta.get("file_path", ""),
            {
                "songTitle": song_prompt.song_title,
                "genreLabel": genre_label,
                "genres": list(genres_for_track),
                "keywords": list(self.keywords),
                "language": self.language,
                "bpm": song_prompt.bpm,
                "keyScale": song_prompt.key_scale,
                "duration": song_prompt.duration,
                "tags": song_prompt.tags,
                "lyrics": song_prompt.lyrics,
                "style": song_prompt.style,
    ...
    async def _make_replay_track(self) -> TrackInfo | None:
        """Build a playable TrackInfo from a random library entry, or None.

        Used for instant session start and for bridging generation gaps. The
        audio bytes are loaded into audio_cache so the normal /api/audio/{id}
        serving path works unchanged.
        """
        genre_ids = [] if self._random_genre else list(self.genres)
        entry = self.library.pick(genre_ids)
        if not entry:
            return None
        track_id = entry["trackId"]
        audio_bytes = await self.library.load_audio(track_id)
        if not audio_bytes:
            return None
        ...
        track = TrackInfo(
            id=track_id,
            song_title=entry.get("songTitle", "Untitled"),
            genre=entry.get("genreLabel", ""),
            is_random=False,
            tags=entry.get("tags", ""),
            lyrics=entry.get("lyrics", ""),
            bpm=int(entry.get("bpm", 90)),
            key_scale=entry.get("keyScale", "C Major"),
            duration=int(entry.get("duration", 60)),
            audio_url=f"/api/audio/{track_id}",
            dj_name=entry.get("djName", ""),
            dj_keywords=list(entry.get("keywords", [])),
            dj_language=entry.get("language", ""),
            replay=True,
        )
        self.track_info_cache[track_id] = track

        # Allow reactions on replayed tracks too
        self.reaction_metadata_cache.setdefault(track_id, {
            "trackId": track_id,
            ...
            "mood": entry.get("mood", ""),
            "vocalStyle": entry.get("vocalStyle", ""),
